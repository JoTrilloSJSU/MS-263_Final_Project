function temperatures = tempcompmatrix(P_cells, JAC_T_cells, setupfilestr, fs_fast, starting_depth, binsize)
    % computes the average temperature at specified depths of interest
    ...
    temperatures = nan(num_datasets, length(depths_of_interest)); % Use NaN for uninitialized values
    ...
    for dataset_idx = 1:num_datasets
        ...
        for i = 1:length(depths_of_interest)
            if temp_counts(i) > 0
                temperatures(dataset_idx, i) = temp_sums(i) / temp_counts(i); % Calculate average
            end
        end
        
        % Display the results for this dataset
        for i = 1:length(depths_of_interest)
            if ~isnan(temperatures(dataset_idx, i))
                fprintf('Dataset %d: Depth: %d dbar, Average Temperature: %.2f°C\n', dataset_idx, depths_of_interest(i), temperatures(dataset_idx, i));
            else
