...
% * 2024-10-15 (rkw).  Updated FM based on get_diss_odas_lueck (Anneke
% email).  All edits have %rkw comment

function diss = get_diss_odas_rkw(shear, A, varargin)
...
diss.mad          = diss.e;
diss.FM           = diss.e;
diss.FM_new       = diss.e;   %rkw (new = Atomix paper Lueck 24)
diss.sigma_e      = diss.e;   %rkw (%statistical uncertainty of ln of estimated dissipation
diss.sigma_spec   = diss.e;   % (%statistical uncertainty of ln of spectral estimate, Atomix Eq 17)
diss.N_s          = diss.e;   % (spectral values in inertial subrange fit (Atomix section 3.4.4)
diss.var_res      = diss.e;   % (variance resolved)
...
% dof_spec = 1.9*num_of_ffts; % Nuttall (1971), % (moved later)

% ----------
N_V = 0;        % Number of vibration signals used for cleaning
if goodman, N_V = size(A,2); end
%--------------
...
while select(end) <= size(shear,1)
    ...
    for column_index = 1:num_of_shear
        ...
        if e_1 < e_inertial_sr % Use the variance method
            ...
            x_limit = K(Range(end))*sqrt(sqrt(nu^3 / e_4));    %rkw, nondimensional value of upper wavenumber of spectral integration (Sec 3.4.1)
            variance_resolved = Nasmyth_area(x_limit);         %rkw Lueck 2022b Equation 20, integral

            N_s = length(Range);
        end
        

        %rkw ----------------------- Statistical uncertainty of spectral
        %estimate (Section 3.4.2 Atomix paper)
        L_f = (diss_length / fs_fast) * W / (nu^3/e_4)^(1/4);
        L_f = L_f * variance_resolved^(3/4);            % eq. 29, Lueck et al 2024
        sigma_e(column_index) = sqrt(5.5 / (1 + (L_f/4)^(7/9)));  %statistical uncertainty of log of estimated dissipation (Atomix 3.4.3)
        var_resolved(column_index) = variance_resolved;
        %---------------------------
...
%        dof(column_index) = dof(column_index) * length(Range - 1);
        ...
        N_s(column_index) = length(Range)-1;  %rkw (based on Anneke code)
        dof_spec(column_index) = 1.9*num_of_ffts; % Nuttall (1971), %rkw (moved from earlier and added column index)

        %rkw---------------------------
        dof_e(column_index) = dof_e(column_index)*length(Range)/length(K);
        FM(column_index) = mad(column_index) .* sqrt(dof_spec(column_index));

        %in mad_ln below, can use any model spectrum so used Nasymth
        %(Atomix paper says different models vary by only ~15%
        mad_ln(column_index)   = mean(abs(log(This_spectrum ./ This_Nasmyth)));      
        sigma_spec_expected(column_index) = sqrt(1.25 * (num_of_ffts - N_V)^(-7/9));  %Equation 17 Atomix (uncertainty of spectral estimate)
        sigma_95 = sigma_spec_expected(column_index) * (0.798 + 1.25 ./ sqrt(N_s(column_index)));
        FM_new(column_index)= mad_ln(column_index) ./ sigma_95; % eq. 22 Lueck et al 2024
        if use_isr
           sigma_e(column_index) = (3/2)*sigma_spec_expected(column_index)/sqrt(N_s(column_index));
        end
        %-------------------------------
    ...
    diss.mad         (:,index)     = mad;
%     diss.FM          (:,index)     = mad .* sqrt(dof_spec);  %rkw
    diss.FM          (:,index)     = FM;  %rkw
    diss.FM_new      (:,index)     = FM_new;   %rkw
    diss.sigma_e     (:,index)     = sigma_e;  %rkw (uncertainty of diss estimate)
    diss.sigma_spec  (:,index)     = sigma_spec_expected;  %rkw (uncertainty of shear spectrum estimate)
    diss.N_s         (:,index)     = N_s;     %rkw (spectral estimates in inertial subrange)
    diss.var_res     (:,index)     = var_resolved;   %rkw (variance resolved)
