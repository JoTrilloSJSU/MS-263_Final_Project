...
% rkw 10/15/2024 - updated get_diss_odas to get_diss_odas_rkw, which
% utilizes new FM calculation based on ATOMIX (Lueck et al. 24) paper




function result = quick_look_rkw( fname, P_start, P_end, varargin )
...
if ~isempty(range) && ~isempty(SH)
    ...
    if exist('EMC_Cur','var') && (length(EMC_Cur) == size(AA,1)) % if there is an EMC
        diss = get_diss_odas_rkw(SH_HP(range,:), [AA(range,:) EMC_Cur(range)], info);
    else
        diss = get_diss_odas_rkw(SH_HP(range,:),  AA(range,:),                 info);
    end
...
if ~isempty(SH_HP)
    diss = get_diss_odas_rkw(SH_HP, AAA, info);
end
